...
var app = http.createServer(function(request , response){
  ...
  if(pathname === '/'){
    ...
    db.query(`SELECT * FROM ${tablename}` , function(error , result){
      ...
      var list = templateView.list(result , pathname);
      var html = templateView.frame(list , pathname);
      response.writeHead(200);
      response.end(html);
    })
    ...
    var html =templateView.writeform(pathname);
    response.writeHead(200);
    response.end(html);
  ...
  request.on('end', function (){
    ...
    if(check != false){
      ...
      function(error2 , result){
        ...
        response.writeHead(302, {Location: `${path_herf}/view?index=${post.id}`});
        response.end();
      }
    )//query
